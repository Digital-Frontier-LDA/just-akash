...
- Tests: full suite at 864 passing (up from 848) — 16 new tests pinning the sweep's service-name identification, dseq-based age gate, and fail-safe classification (young probe spared, unknown-age spared, real workloads never reaped, dry-run destroys nothing, best-effort on API failure).
