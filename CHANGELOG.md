...
### Fixed
- **The `benchmark` command never actually ran — it produced no output on any provider.** It sent BENCH_SH down `exec()`, which uses the provider's *argv* path: `$()`, pipes, `;` and newlines come back literal, so a shell script yields nothing. (Shipped in #61 with unit tests only; verified now against a live provider that it returned zero metrics.) Added `LeaseShellTransport.exec_shell_script()`, which runs a script via `sh -c`, and pointed the benchmark at it. Live: the benchmark now returns real hardware (AMD EPYC 7502P, 8.4 GB/s RAM, ~1.3ms RTT, etc.).
