...
- Tests: full suite at 866 passing (up from 848) — 18 new tests pinning the sweep's service-name identification, dseq-based age gate, and fail-safe classification (young probe spared, unknown-age spared, real workloads never reaped, dry-run destroys nothing and says so, an un-destroyable orphan is surfaced not hidden, a genuine inspection failure marks the sweep incomplete while a 404 counts as already-gone, best-effort on API failure).
