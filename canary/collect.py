...
def merge(
    ...
    *,
    ...
    `deployed` is whether a canary DEPLOYMENT EXISTS on this provider. It is deliberately
    distinct from `reachable`: a provider with no canary is not an unreachable provider,
    it is a provider we never tried.

    IT MUST NOT BE INFERRED FROM THE PRESENCE OF A URI. `ensure.plan()` deliberately
    carries the PREVIOUS uri forward for a provider whose canary is GONE, so the collector
    records the outage against the right endpoint -- which means a dead canary keeps a
    real-looking endpoint by design, and `bool(uri)` reports deployed=1 for exactly the
    state this metric exists to expose. Callers pass the targets file's `live` flag, which
    `ensure.plan()` writes from what it actually observed on chain.

    `reachable` still IMPLIES deployed, and that direction stays enforced: a scrape that
    succeeded proves a deployment exists whatever the flag says, and letting an explicit
    False win over a successful scrape would publish deployed=0 for a provider that is
    serving -- which pages AkashCanaryNoDeployments while every canary answers. Only the
    other direction was wrong.

    `deployed=None` means the caller does not track targets and defaults to TRUE, not to
    `reachable`. Deferring to `reachable` looks conservative and is the opposite: it makes
    an unreachable scrape indistinguishable from a provider that was never deployed, so
    the outage takes the no-scrape-attempted branch below and is silently dropped instead
    of counted.
    """
    ...
    # ASSIGNMENT, never setdefault: setdefault would latch the first value forever, so a
    # provider whose canary later disappeared would go on publishing deployed=1.
    deployed_resolved = bool(reachable or (True if deployed is None else deployed))
    p["deployed"] = int(deployed_resolved)
    p["last_collect"] = now

    if not deployed_resolved:
        # NO SCRAPE WAS ATTEMPTED, so nothing derived from one may move -- not
        # `checks_total`, not `unreachable_checks_total`, not `reachable`, not
        # `scrape_seconds`. `unreachable_checks_total` is cumulative and never resets, so
        # advancing it for a provider that was never scraped permanently poisons rate()
        # availability math; and `reachable 0` is a measurement we did not take, which is
        # the false page this whole signal exists to stop. render() withholds the series
        # rather than publishing a zero for them.
...
def render(
    ...
    # EVERY per-provider scrape-derived series below is gated on `deployed`. A provider
    # with no canary publishes `akash_canary_deployed 0` and NOTHING else.
    #
    # Absence is what stops the page, and it stops it with NO change in df-grafana. Both
    # paging rules there are LEFT-GATED on akash_canary_scrape_seconds and then match
    # `on(provider) akash_canary_reachable == 0`
    # (alert.rules.yml AkashCanaryUnreachable / AkashCanaryScrapeTimingOut), so an absent
    # scrape_seconds makes the vector selector yield nothing and neither rule can fire.
    # Publishing `reachable 0` keeps both firing for a provider that never had a canary,
    # until somebody edits the alert in another repo -- cross-repo coordination during an
    # incident, which is what made this expensive the first time.
    #
    ...
    add(
        "# HELP akash_canary_providers_total Providers the canary is responsible for. The "
        "denominator that makes PARTIAL fleet loss expressible -- active_deployments alone "
...
def main() -> int:
    ...
    for provider, t in sorted(targets.items()):
        uri = t.get("uri") or ""
        # `live`, NOT bool(uri). ensure.plan() carries the previous uri forward for a
        # canary that is GONE, so bool(uri) reports deployed=1 for exactly the state this
        # signal exists to expose -- and then scrapes that dead endpoint, producing a real
        # scrape_seconds and reachable=0, which is what AkashCanaryUnreachable pages on.
        # The `bool(uri)` fallback is migration-only: it covers the first run against a
        # targets.json ensure.py has not rewritten yet, and reproduces the old behaviour.
        deployed = bool(t.get("live", bool(uri)))
        should_scrape = bool(uri) and deployed
        ok, body, elapsed = scrape(uri, a.timeout) if should_scrape else (False, "", 0.0)
        state = merge(
            state,
            provider,
            str(t.get("dseq") or ""),
